resource "openstack_networking_port_v2" "tf-opensdn-controller-eth0" {
  name       = "${var.resource_prefix}-opensdn-controller-eth0"
...
resource "openstack_compute_instance_v2" "tf-opensdn-controller" {
  ...
  name                    = "${var.resource_prefix}-opensdn-controller"
  ...
  network {
    port = openstack_networking_port_v2.tf-opensdn-controller-eth0.id
  }
